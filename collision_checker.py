    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                circle_locations[:,0]=path[0][j]+self._circle_offsets[0]*cos(path[2][j])
                circle_locations[:,1]=path[1][j]+self._circle_offsets[1]*cos(path[2][j])
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                weight_x=3
                weight_y=3
                score = weight_x*sqrt(abs(paths[i][0][-1]**2-goal_state[0]**2))+weight_y*sqrt(abs(paths[i][1][-1]**2-goal_state[1]**2))
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # score += self._weight * ...
                            # --------------------------------------------------
                            if paths[i][0][-1]-goal_state[0]<0:
                                score=score*1.5
                            if paths[i][1][-1]-goal_state[1]<0:
                                score=score*1.5
                            score+=self._weight*len(paths)/abs(j-i)
